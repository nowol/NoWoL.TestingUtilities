...
#pragma warning restore IDE0060 // Remove unused parameter
        ...
        public void AnalyzeActionThrowsIfExpressionIsNull()
        {
            ...
            var ex = Assert.Throws<ArgumentNullException>(() => sut.Analyze((Expression<Action<TestClass>>)null));
        ...
        {
            var sut = new ExpressionArgumentsAnalyzer();
            var ex = Assert.Throws<ArgumentNullException>(() => sut.Analyze((Expression<Func<TestClass, int>>)null));
            Assert.Equal("expression",
                         ex.ParamName);
        }
        ...
        {
            var sut = new ExpressionArgumentsAnalyzer();
            var info = sut.Analyze<TestClass, int>(x => x.SomeMethod(23,
                                                                     "Text",
                                                                     25));
            ...
            Assert.Equal("i",
                         info.ArgumentValues[0].Name);
            Assert.Equal(23,
                         info.ArgumentValues[0].Value);
            Assert.Equal("s",
                         info.ArgumentValues[1].Name);
            Assert.Equal("Text",
                         info.ArgumentValues[1].Value);
            Assert.Equal("d",
                         info.ArgumentValues[2].Name);
            Assert.Equal(25d,
                         info.ArgumentValues[2].Value);
        }
        ...
        {
            var sut = new ExpressionArgumentsAnalyzer();
            var info = sut.Analyze<TestClass>(x => x.SomeVoidMethod(23,
                                                                    "Text",
                                                                    25));
